#!/usr/bin/env python
# -*- coding: utf-8 -*-
""" Discord bot for Echo Master League Management """
...
# connect to google sheets via the google cloud services api
...
async def help_command(interaction: discord.Interaction):
    embed = Embed(
        title="Echo Master League Commands",
        description="Here are some useful commands in Echo Master League Discord server:",
        color=0x3498DB,
    )  # You can customize the color code

    # Add fields for each command
    embed.add_field(
        name="/ranks", value="Check the latest EML team rankings.", inline=False
    )
    embed.add_field(
        name="/matches",
        value="View the matches and results for the current week.",
        inline=False,
    )
    embed.add_field(
        name="/rosters", value="Explore the rosters of teams in EML.", inline=False
    )
    embed.add_field(
        name="/registration",
        value="Register for Echo Master League and join the competition.",
        inline=False,
    )
    embed.add_field(
        name="/website",
        value="Visit the official Echo Master League website for more information.",
        inline=False,
    )
    embed.add_field(
        name="/league_rules",
        value="Read the rules and guidelines for Echo Master League.",
        inline=False,
    )
    embed.add_field(
        name="/server_coc",
        value="Review the Code of Conduct for the EML Discord server.",
        inline=False,
    )
    embed.add_field(
        name="/ticket",
        value="Submit a ticket for assistance, to report an issue, or to schedule your matches.",
        inline=False,
    )
    embed.add_field(
        name="/support",
        value="Visit the support channel for assistance in the EML Discord server.",
        inline=False,
    )
    embed.add_field(
        name="/staff_app", value="Apply to become part of EML Staff.", inline=False
    )
    ...
    embed.add_field(
        name="/action_list", value="View the EML Action List.", inline=False
    )
    embed.add_field(
        name="/lounge_report",
        value="Report gameplay violations in the Echo VR Lounge.",
        inline=False,
    )
    embed.add_field(
        name="/list_members", value="List members with specified roles.", inline=False
    )
...
async def matches(interaction: discord.Interaction):
    embed = Embed(
        title="Current Week Matches and Results",
...
async def website(interaction: discord.Interaction):
    embed = Embed(
        title="Echo Master League Website",
...
async def server_coc(interaction: discord.Interaction):
    embed = Embed(
        title="Discord Server Code of Conduct",
